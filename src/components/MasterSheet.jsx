...
function MasterSheet() {
  ...
  const getRobloxData = async (username) => {
    ...
    try {
      // Try RoProxy first (reliable proxy for Roblox APIs)
      const proxyUrl = `https://users.roproxy.com/v1/usernames/users`
      ...
      if (!response.ok) {
        // Fallback: try direct Roblox API
        const directResponse = await fetch('https://users.roblox.com/v1/usernames/users', {
          method: 'POST',
          headers: { 
            'Content-Type': 'application/json',
            'Accept': 'application/json'
        ...
        if (!directResponse.ok) {
          throw new Error(`Could not find Roblox user "${cleanUsername}". The user may not exist, or Roblox's API is temporarily unavailable.`)
        }

        const directData = await directResponse.json()
        if (!directData.data || directData.data.length === 0) {
          throw new Error(`Roblox user "${cleanUsername}" not found. Please check the spelling.`)
        }

        const user = directData.data[0]
      ...
      throw new Error(`Network error while fetching Roblox data. Please try again. Error: ${err.message}`)
    }
  }

  const getAvatarUrl = async (userId) => {
    try {
      // Try RoProxy for avatar
      const proxyResponse = await fetch(`https://thumbnails.roproxy.com/v1/users/avatar-headshot?userIds=${userId}&size=150x150&format=Png&isCircular=false`)
      if (proxyResponse.ok) {
      ...
      // Fallback to direct
      const directResponse = await fetch(`https://thumbnails.roblox.com/v1/users/avatar-headshot?userIds=${userId}&size=150x150&format=Png&isCircular=false`)
      if (directResponse.ok) {
          ...
          <div style={{ display: 'grid', gap: '16px' }}>
            <div>
              ...
              <input
                ...
                className="crf-input"
                placeholder="Enter Roblox username (no @ needed)"
                required
                disabled={!!player}
              />
              {!player && (
                <p style={{ fontSize: '0.75rem', color: '#a0a0a0', marginTop: '4px' }}>
                  Avatar and User ID will be fetched automatically. Just type the username.
                </p>
              )}
            </div>
